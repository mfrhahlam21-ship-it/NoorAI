...
شكوكي / سؤالي: ${prompt || "كيف أحصل على أفضل أداء للـ FPS والـ Ping؟"}`;
  ...
  // Serve launcher installers unconditionally BEFORE any other middleware
  ...
  app.use('/downloads', (req, res, next) => {
    ...
    console.log(`NoorAI Server: Received download request for "${filename}"`);

    // Security: prevent directory traversal
    if (!filepath.startsWith(downloadsPath)) {
      console.warn(`NoorAI Server: Security rejection for directory traversal: "${filepath}"`);
      return res.status(400).json({ error: "Invalid download path requested" });
    }

    if (!fs.existsSync(filepath)) {
      console.warn(`NoorAI Server: Requested download file does not exist: "${filepath}"`);
      return res.status(404).json({ error: "Requested installer file is not currently built on the server. Please compile the installer first." });
    }

    try {
      const stats = fs.statSync(filepath);
      if (!stats.isFile()) {
        console.warn(`NoorAI Server: Path is not a valid file: "${filepath}"`);
        return res.status(400).json({ error: "Requested path is not a file" });
      }

      console.log(`NoorAI Server: Serving binary installer "${filename}" | Size: ${stats.size} bytes`);
      ...
      res.download(filepath, filename, (err) => {
        if (err) {
          console.error(`NoorAI Server: Error during file transmission for "${filename}":`, err);
          if (!res.headersSent) {
            res.status(500).json({ error: "Transmission error during download" });
          }
        }
      });
    } catch (e) {
      console.error(`NoorAI Server: Exception while processing download:`, e);
      if (!res.headersSent) {
        res.status(500).json({ error: "Internal server error during download" });
      }
    }
  });
  console.log(`NoorAI Server: Mounted unconditional launcher /downloads provider path at ${downloadsPath}`);
