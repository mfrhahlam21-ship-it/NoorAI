...
function createWindow() {
  ...
  const webUrl = process.env.LAUNCHER_WEB_URL || "http://localhost:3000";
...
import_electron.ipcMain.handle("get-system-info", async () => {
  ...
  return { osInfo, cpu, mem, graphics };
});
import_electron.ipcMain.handle("scan-games", async () => {
  const locations = [
  ...
  for (const loc of locations) {
    try {
      if (import_fs.default.existsSync(loc)) {
        const items = import_fs.default.readdirSync(loc, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
        for (const it of items) {
          found.push(import_path.default.join(loc, it));
        }
      }
    } catch (e) {
    }
...
import_electron.ipcMain.handle("check-drivers", async () => {
  const graphics = await import_systeminformation.default.graphics();
  const controller = graphics.controllers && graphics.controllers[0];
  return controller || {};
});
import_electron.ipcMain.handle("clean-shader-cache", async (_event, paths) => {
  const results = [];
...
import_electron.ipcMain.handle("analyze-crash", async (_event, reportText) => {
  const txt = (reportText || "").toLowerCase();
  let summary = "No clear pattern detected.";
  if (txt.includes("dxgi_error_device_removed")) summary = "GPU device removed / driver crash suspected.";
  if (txt.includes("out of memory")) summary = "VRAM or memory allocation failure.";
  return { summary };
});
import_electron.ipcMain.handle("get-fps", async () => {
  return { fps: Math.floor(30 + Math.random() * 90) };
});
